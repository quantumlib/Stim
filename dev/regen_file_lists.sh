...
find src | grep "\\.cc$" | grep -v "\\.\(test\|perf\|pybind\)\\.cc$" | grep -v "src/main\\.cc" | LC_ALL=C sort > "file_lists/source_files_no_main"
find src | grep "\\.test\\.cc$" | LC_ALL=C sort > "file_lists/test_files"
find src | grep "\\.perf\\.cc$" | LC_ALL=C sort > "file_lists/benchmark_files"
find src | grep "\\.pybind\\.cc$" | LC_ALL=C sort > "file_lists/python_api_files"
