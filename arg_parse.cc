...
int find_int_argument(
    const char *name, int default_value, int min_value, int max_value, int argc,
    const char **argv) {
  const char *text = find_argument(name, argc, argv);
  if (text == nullptr) {
  ...
  if (i < min_value || i > max_value) {
    fprintf(stderr, "\033[31mInteger value '%s' for flag '%s' doesn't satisfy %d <= %ld <= %d.\033[0m\n", text,
        name, min_value, i, max_value);
    exit(EXIT_FAILURE);
  }
...
float find_float_argument(
  ...
  if (f < min_value || f > max_value || f != f) {
    fprintf(stderr, "\033[31mFloat value '%s' for flag '%s' doesn't satisfy %f <= %f <= %f.\033[0m\n", text,
        name, min_value, f, max_value);
    exit(EXIT_FAILURE);
  }
