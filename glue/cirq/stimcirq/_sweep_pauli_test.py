...
def test_repr():
    cirq.testing.assert_equivalent_repr(
        stimcirq.SweepPauli(
            stim_sweep_bit_index=1, pauli=cirq.X, cirq_sweep_symbol="a"
        ),
        global_vals={"stimcirq": stimcirq},
    )
    cirq.testing.assert_equivalent_repr(
        stimcirq.SweepPauli(
            stim_sweep_bit_index=2, pauli=cirq.Y, cirq_sweep_symbol="b"
        ),
        global_vals={"stimcirq": stimcirq},
    )
...
def test_resolving_without_sweep_resolver():
    ...
    (
        op0,
        op1,
    ) = resolved_circuit.all_operations()
    assert isinstance(
        op0.gate, stimcirq.SweepPauli
    )  # Operation should still be a SweepPauli
    assert isinstance(
        op1.gate, type(cirq.X)
    )  # Operation should be converted to a cirq.X
...
def test_cirq_compatibility():
    circuit = cirq.Circuit(
        stimcirq.SweepPauli(
            stim_sweep_bit_index=5, pauli=cirq.X, cirq_sweep_symbol="xx"
        ).on(cirq.LineQubit(0)),
        cirq.measure(cirq.LineQubit(0), key="k"),
    )
    np.testing.assert_array_equal(
        cirq.Simulator().sample(circuit, params={"xx": 0}, repetitions=3)["k"],
        [0, 0, 0],
    )
    np.testing.assert_array_equal(
        cirq.Simulator().sample(circuit, params={"xx": 1}, repetitions=3)["k"],
        [1, 1, 1],
    )

    circuit = cirq.Circuit(
        stimcirq.SweepPauli(
            stim_sweep_bit_index=5, pauli=cirq.Z, cirq_sweep_symbol="xx"
        ).on(cirq.LineQubit(0)),
        cirq.measure(cirq.LineQubit(0), key="k"),
    )
    np.testing.assert_array_equal(
...
def test_json_backwards_compat_exact():
    raw = stimcirq.SweepPauli(
        stim_sweep_bit_index=2, pauli=cirq.Z, cirq_sweep_symbol="abc"
    )
