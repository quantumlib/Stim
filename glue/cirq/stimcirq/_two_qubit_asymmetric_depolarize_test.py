...
def test_json_backwards_compat_exact():
    raw = stimcirq.TwoQubitAsymmetricDepolarizingChannel(
        [0.0125, 0.1, 0, 0.23, 0, 0, 0.0375, 0, 0.01, 0, 0, 0, 0, 0.25, 0]
    )
...
def test_native_cirq_gate_converts():
    c = cirq.Circuit(
        cirq.asymmetric_depolarize(error_probabilities={"IX": 0.125, "ZY": 0.25}).on(
