...
def test_simulation():
    ...
    s = cirq.Simulator().sample(
        cirq.Circuit(cirq.X(a), cirq.measure(a, key="a"), stimcirq.ShiftCoordsAnnotation((1, 2, 3))),
        repetitions=3
    )
...
def test_diagram():
    ...
    cirq.testing.assert_has_diagram(
        cirq.Circuit(cirq.measure(a, key="a"), cirq.measure(b, key="b"), stimcirq.ShiftCoordsAnnotation([1, 2, 3])),
        """
0: ---M('a')---------------
...
def test_equality():
    ...
    eq.add_equality_group(stimcirq.ShiftCoordsAnnotation([1, 2]), stimcirq.ShiftCoordsAnnotation((1, 2)))
