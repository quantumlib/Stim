...
def text_diagram_to_networkx_graph(
    text_diagram: str, *, value_func: Callable[[str], K] = str
) -> nx.MultiGraph:
...
def _find_all_edges(
    char_map: Dict[complex, str], terminal_map: Dict[complex, K]
) -> List[Tuple[K, K]]:
    edges = []
    ...
    for xy, c in char_map.items():
        ...
        if dxy is None:
            raise ValueError(
                f"Character {x+1} ('{c}') in line {y+1} isn't part in a node or an edge"
            )
        n1 = _find_end_of_edge(xy + dxy, dxy, char_map, terminal_map, already_travelled)
        n2 = _find_end_of_edge(
            xy - dxy, -dxy, char_map, terminal_map, already_travelled
        )
        edges.append((n2, n1))
    return edges


def _find_end_of_edge(
    xy: complex,
    dxy: complex,
    char_map: Dict[complex, str],
    terminal_map: Dict[complex, K],
    already_travelled: Set[complex],
):
    while True:
        c = char_map[xy]
        ...
        if c == "*":
            for dx2 in [-1, 0, 1]:
                for dy2 in [-1, 0, 1]:
                    ...
                    if (
                        ...
                        and dxy2 != -dxy
                        and c2 is not None
                        and c2 in DIR_TO_CHARS[dxy2]
                    ):
                        next_deltas.append(dxy2)
            if len(next_deltas) != 1:
                raise ValueError(
                    f"Edge junction ('*') at character {int(xy.real)+1}$ in line {int(xy.imag)+1} doesn't have exactly 2 legs."
                )
            (dxy,) = next_deltas
        else:
            ...
            if c not in expected:
                raise ValueError(
                    f"Dangling edge at character {int(xy.real)+1} in line {int(xy.imag)+1} travelling dx=${int(dxy.real)},dy={int(dxy.imag)}."
                )
        xy += dxy


def _find_nodes(
    char_map: Dict[complex, str], value_func: Callable[[str], K]
) -> Tuple[Dict[complex, int], List[K]]:
    node_ids = {}
    nodes = []
