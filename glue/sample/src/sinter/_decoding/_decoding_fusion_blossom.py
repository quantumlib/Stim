...
class FusionBlossomCompiledDecoder(CompiledDecoder):
    def __init__(
        self,
        solver: "fusion_blossom.SolverSerial",
...
class FusionBlossomDecoder(Decoder):
    ...
    def compile_decoder_for_dem(
        ...
        solver, fault_masks = (
            detector_error_model_to_fusion_blossom_solver_and_fault_masks(dem)
        )
    ...
    def decode_via_files(
        ...
        *,
        ...
        solver, fault_masks = (
            detector_error_model_to_fusion_blossom_solver_and_fault_masks(error_model)
        )
        ...
        with open(dets_b8_in_path, "rb") as dets_in_f:
            with open(obs_predictions_b8_out_path, "wb") as obs_out_f:
                for _ in range(num_shots):
                    ...
                    syndrome = fusion_blossom.SyndromePattern(
...
def detector_error_model_to_fusion_blossom_solver_and_fault_masks(
    model: stim.DetectorErrorModel,
) -> Tuple["fusion_blossom.SolverSerial", np.ndarray]:
    ...
    rescaled_edges = [(a, b, round(w * 2**10 / max_weight) * 2) for a, b, w, _ in edges]
