...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [0, 1, 0.1, 0.5],
            1,
            [-np.inf, -np.inf, np.log(0.1 * 0.9 * 2), np.log(0.5)],
        ),
...
def test_binary_search():
    assert (
        binary_search(func=lambda x: x**2, min_x=0, max_x=10**100, target=100.1) == 10
    )
    ...
    assert (
        binary_search(func=lambda x: x**2, min_x=0, max_x=10**100, target=10**300)
        == 10**100
    )
...
def test_least_squares_through_point():
    fit = least_squares_through_point(
        xs=np.array([1, 2, 3]), ys=np.array([2, 3, 4]), required_x=1, required_y=2
    )
    np.testing.assert_allclose(fit.slope, 1)
    np.testing.assert_allclose(fit.intercept, 1)

    fit = least_squares_through_point(
        xs=np.array([1, 2, 3]), ys=np.array([2, 3, 4]), required_x=1, required_y=1
    )
...
def test_binary_intercept():
    t = binary_intercept(
        func=lambda x: x**2, start_x=5, step=1, target_y=82.3, atol=0.01
    )
    ...
    assert t > 0 and abs(t**2 - 82.3) <= 0.01
    t = binary_intercept(
        func=lambda x: x**2, start_x=0, step=-1, target_y=82.3, atol=0.01
    )
    assert t < 0 and abs(t**2 - 82.3) <= 0.01
    t = binary_intercept(
        func=lambda x: -(x**2), start_x=0, step=-1, target_y=-82.3, atol=0.2
    )
