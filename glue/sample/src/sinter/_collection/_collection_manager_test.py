...
def test_manager():
    ...
    t0 = sinter.Task(
        ...
        collection_options=sinter.CollectionOptions(
            max_shots=100_000_000, max_errors=100
        ),
        json_metadata={"a": 3},
    )
    t1 = sinter.Task(
...
"""
    ...
    manager.shared_worker_output_queue.put(
        (
            "computed_strong_id",
            2,
            "c03f7852e4579e2a99cefac80eeb6b09556907540ab3d7787a3d07309c3333aa",
        )
    )
    manager.shared_worker_output_queue.put(
        (
            "computed_strong_id",
            1,
            "a9165b6e4ab1053c04c017d0739a7bfff0910d62091fc9ee81716833eda7f604",
        )
    )
...
"""
    ...
    _assert_drain_queue(
        manager.worker_states[0].input_queue,
        [
            (
                "change_job",
    ...
    _assert_drain_queue(
        manager.worker_states[1].input_queue,
        [
    ...
    _assert_drain_queue(
        manager.worker_states[2].input_queue,
        [
    ...
    assert not log
    _put_wait_not_empty(
        manager.shared_worker_output_queue,
        (
            "flushed_results",
            2,
            (
...
"""
    ...
    assert manager.shared_worker_output_queue.empty()
    _put_wait_not_empty(
        manager.shared_worker_output_queue, ("returned_shots", 2, (t1.strong_id(), 0))
    )
    assert manager.process_message()
    assert manager.shared_worker_output_queue.empty()
    assert (
