...
class _ManagedTaskState:
    def __init__(
        self, *, partial_task: Task, strong_id: str, shots_left: int, errors_left: int
    ):
...
class CollectionManager:
    def __init__(
        ...
        *,
        ...
        self.allowed_cpu_affinity_ids = (
            None
            if allowed_cpu_affinity_ids is None
            else sorted(set(allowed_cpu_affinity_ids))
        )
        ...
        self.shared_worker_output_queue: Optional[
            multiprocessing.SimpleQueue[Tuple[str, int, Any]]
        ] = None
    ...
    def _compute_task_ids(self):
        ...
        while worker_to_task_map or unknown_task_ids:
            while idle_worker_ids and unknown_task_ids:
                ...
                self.worker_states[worker_id].send_message(
                    ("compute_strong_id", self.partial_tasks[unknown_task_id])
                )
            ...
            try:
                ...
                if message_type == "computed_strong_id":
                    ...
                    self.task_strong_ids[worker_to_task_map.pop(worker_id)] = (
                        message_body
                    )
                    idle_worker_ids.append(worker_id)
                elif message_type == "stopped_due_to_exception":
                    cur_task, cur_shots_left, unflushed_work_done, traceback, ex = (
                        message_body
                    )
        ...
        for k in range(len(self.partial_tasks)):
            options = self.partial_tasks[k].collection_options.combine(
    ...
    def _handle_task_progress(self, task_id: Any):
        ...
        if task_state.is_completed():
            workers_ready = all(
                self.worker_states[worker_id].has_returned_all_shots()
    ...
    def process_message(self) -> bool:
        ...
        if message_type == "flushed_results":
            ...
            if self.custom_error_count_key is None:
                ...
                task_state.errors_left -= anon_stat.custom_counts[
                    self.custom_error_count_key
                ]
        ...
        while idle_workers:
            ...
            worker_state.send_message(
                (
                    "change_job",
                    (
                        task_state.partial_task,
                        CollectionOptions(max_errors=task_state.errors_left),
                        task_state.assigned_soft_error_flush_threshold,
                    ),
                )
            )

    def _distribute_unassigned_work_to_workers_within_a_job(
        self, task_state: _ManagedTaskState
    ):
        if (
            not self.started
            or not task_state.workers_assigned
            or task_state.shots_left <= 0
        ):
            return

        num_task_workers = len(task_state.workers_assigned)
        expected_shots_per_worker = (
            task_state.shots_left + num_task_workers - 1
        ) // num_task_workers

        # Give unassigned shots to idle workers.
        for worker_id in sorted(
            task_state.workers_assigned,
            key=lambda wid: self.worker_states[wid].assigned_shots,
        ):
            worker_state = self.worker_states[worker_id]
            if worker_state.assigned_shots < expected_shots_per_worker:
                shots_to_assign = min(
                    expected_shots_per_worker - worker_state.assigned_shots,
    ...
    def status_message(self) -> str:
        ...
        if num_known_tasks_ids < len(self.task_strong_ids):
            return (
                f"Analyzed {num_known_tasks_ids}/{len(self.task_strong_ids)} tasks..."
            )
        ...
        for k, strong_id in enumerate(self.task_strong_ids):
            ...
            c_errors = (
                c.custom_counts[self.custom_error_count_key]
            ...
            line = [
                ...
                (
                    ...
                    if dt is None or dt == 0
                        ...
                        "[draining]"
                        if dt <= 0
                        else "<1m" if dt < 1 else str(round(dt)) + "m"
                    )
                )
                + ("·∞" if w == 0 else ""),
        ...
        if lines:
            ...
            justs = cast(
                ...
                [str.rjust, str.rjust, str.rjust, str.rjust, str.rjust, str.ljust],
            )
            cols = len(lines[0])
            lengths = [
    ...
    def _update_soft_error_threshold_for_a_job(self, task_state: _ManagedTaskState):
        if task_state.errors_left <= len(task_state.workers_assigned):
            ...
            task_state.errors_left
            <= task_state.assigned_soft_error_flush_threshold * self.num_workers
        ):
            desired_threshold = max(
                1, math.ceil(task_state.errors_left * 0.5 / self.num_workers)
            )
        else:
            return
        ...
        if task_state.assigned_soft_error_flush_threshold != desired_threshold:
            ...
            for wid in task_state.workers_assigned:
                self.worker_states[wid].send_message(
                    ("set_soft_error_flush_threshold", desired_threshold)
                )
    ...
    def _take_work_if_unsatisfied_workers_within_a_job(
        ...
        if all(
            self.worker_states[w].assigned_shots > 0
            for w in task_state.workers_assigned
        ):
            return
        ...
        for worker_id in sorted(
            task_state.workers_assigned,
            key=lambda w: self.worker_states[w].assigned_shots,
            reverse=True,
        ):
            worker_state = self.worker_states[worker_id]
            if (
                worker_state.asked_to_drop_shots
                or worker_state.assigned_shots <= expected_shots_per_worker
            ):
                continue
