...
def test_repr():
    ...
    v = sinter.Task(
        circuit=circuit, detector_error_model=circuit.detector_error_model()
    )
    assert eval(repr(v), {"stim": stim, "sinter": sinter, "np": np}) == v
    ...
    v = sinter.Task(
        circuit=circuit, postselected_observables_mask=np.array([1], dtype=np.uint8)
    )
    assert eval(repr(v), {"stim": stim, "sinter": sinter, "np": np}) == v

    v = sinter.Task(
        circuit=circuit, collection_options=sinter.CollectionOptions(max_shots=10)
    )
    assert eval(repr(v), {"stim": stim, "sinter": sinter, "np": np}) == v
