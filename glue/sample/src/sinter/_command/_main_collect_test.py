...
def test_split_by():
    assert split_by("abcdefcccghi", lambda e: e == "c") == [
        list("ab"),
        list("c"),
        list("def"),
        list("ccc"),
...
def test_main_collect_post_select_observables():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert (
            0.015625 - 0.01
            <= stats.errors / (stats.shots - stats.discards)
            <= 0.015625 + 0.02
        )
...
def test_main_collect_count_observable_error_combos():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert len(data) == 1
        (item,) = data
        assert set(item.custom_counts.keys()) == {
            "obs_mistake_mask=E_",
            "obs_mistake_mask=_E",
            "obs_mistake_mask=EE",
        }
        assert (
            0.1 * 0.8 - 0.01
            < item.custom_counts["obs_mistake_mask=_E"] / item.shots
            < 0.1 * 0.8 + 0.01
        )
        assert (
            0.9 * 0.2 - 0.01
            < item.custom_counts["obs_mistake_mask=E_"] / item.shots
            < 0.9 * 0.2 + 0.01
        )
        assert (
            0.1 * 0.2 - 0.01
            < item.custom_counts["obs_mistake_mask=EE"] / item.shots
            < 0.1 * 0.2 + 0.01
        )
...
def test_main_collect_count_detection_events():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert (
            100000 * 0.1 * 0.5
            < item.custom_counts["detection_events"]
            < 100000 * 0.1 * 1.5
        )
