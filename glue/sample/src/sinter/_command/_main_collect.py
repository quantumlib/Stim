...
def iter_file_paths_into_goals(
    circuit_paths: Iterator[str],
    metadata_func: Callable,
    postselected_detectors_predicate: Optional[
        Callable[[int, Any, Tuple[float, ...]], bool]
    ],
    ...
    for path in circuit_paths:
        ...
        if postselected_detectors_predicate is not None:
            post_mask = post_selection_mask_from_predicate(
                circuit,
                metadata=metadata,
        ...
        if any(postselected_observables):
            postselected_observables_mask = np.zeros(
                shape=math.ceil(circuit.num_observables / 8), dtype=np.uint8
            )
...
def parse_args(args: List[str]) -> Any:
    parser = argparse.ArgumentParser(
        description="Collect Monte Carlo samples.", prog="sinter collect"
    )
    parser.add_argument(
        "--circuits",
        nargs="+",
    ...
    parser.add_argument(
        "--postselected_detectors_predicate",
        type=str,
        default="""False""",
    ...
    parser.add_argument(
        "--quiet", help="Disables writing progress to stderr.", action="store_true"
    )
    parser.add_argument(
    ...
    a.postselected_observables_predicate = eval(
        compile(
    ...
    if a.postselected_detectors_predicate == "False":
        if a.postselect_detectors_with_non_zero_4th_coord:
            a.postselected_detectors_predicate = lambda index, metadata, coords: coords[
                3
            ]
        else:
        ...
        if a.postselect_detectors_with_non_zero_4th_coord:
            raise ValueError(
                "Can't specify both --postselect_detectors_with_non_zero_4th_coord and --postselected_detectors_predicate"
            )
        a.postselected_detectors_predicate = eval(
            compile(
                "lambda index, metadata, coords: "
                + cast(str, a.postselected_detectors_predicate),
    ...
    if a.custom_decoders_module_function is not None:
        ...
        for entry in a.custom_decoders_module_function:
            ...
            if len(terms) != 2:
                raise ValueError(
                    "--custom_decoders_module_function didn't have exactly one colon "
    ...
    for decoder in a.decoders:
        if decoder not in BUILT_IN_SAMPLERS and (
            a.custom_decoders is None or decoder not in a.custom_decoders
        ):
            ...
            if a.custom_decoders is None:
                ...
                message += (
                    f"Available custom decoders: {sorted(a.custom_decoders.keys())}."
                )
    ...
    if a.allowed_cpu_affinity_ids is not None:
        ...
        for e in a.allowed_cpu_affinity_ids:
            try:
                ...
                raise ValueError(
                    "Failed to eval {e!r} for --allowed_cpu_affinity_ids"
                ) from ex
...
def main_collect(*, command_line_args: List[str]):
    ...
    print_to_stdout = (
        args.also_print_results_to_stdout or args.save_resume_filepath is None
    )
    ...
    if args.processes == "auto":
        ...
        if num_workers < 1:
            raise ValueError(
                f'--processes must be a non-negative integer, or "auto", but was: {args.processes}'
            )
    try:
        collect(
