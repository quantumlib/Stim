...
def parse_args(args: List[str]) -> Any:
    parser = argparse.ArgumentParser(
        description="Plot collected CSV data.", prog="sinter plot"
    )
    parser.add_argument(
        "--filter_func",
        type=str,
    ...
    parser.add_argument(
        "--in", type=str, nargs="+", required=True, help="Input files to get data from."
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--title", default=None, type=str, help="Sets the title of the plot."
    )
    parser.add_argument(
        "--subtitle",
        default=None,
    ...
    parser.add_argument(
        "--highlight_max_likelihood_factor",
        type=float,
        default=1000,
    ...
    parser.add_argument(
        "--line_fits", action="store_true", help="Adds dashed line fits to every curve."
    )
    ...
    if (len(a.type) == 0 and a.y_func is not None) or list(a.type) == "custom_y":
        if a.failure_units_per_shot_func is not None:
            raise ValueError(
                "--failure_units_per_shot_func doesn't affect --type custom_y"
            )
    if (a.failure_units_per_shot_func is not None) != (a.failure_unit_name is not None):
        raise ValueError(
            "--failure_units_per_shot_func and --failure_unit_name can only be specified together."
        )
    if a.failure_values_func is not None and a.failure_units_per_shot_func is None:
        raise ValueError(
            "Specified --failure_values_func without --failure_units_per_shot_func"
        )
    ...
    a.preprocess_stats_func = (
        None
        if a.preprocess_stats_func is None
        else eval(
            compile(
    ...
    a.failure_units_per_shot_func = _compile_argument_into_func(
        "failure_units_per_shot_func"
    )
    a.failure_values_func = _compile_argument_into_func("failure_values_func")
    raw_plot_args_func = eval(
        compile(
...
def _set_axis_scale_label_ticks(
    *,
    ...
    set_lim = cast(
        ...
        ax.set_ylim if y_not_x else ax.set_xlim,
    )
...
def _plot_helper(
    *,
    ...
    total.data = {k: v for k, v in total.data.items() if bool(filter_func(v))}
    ...
    num_plots = (
        include_error_rate_plot + include_discard_rate_plot + include_custom_plot
    )
    ...
    plotted_stats: List["sinter.TaskStats"] = [stat for stat in total.data.values()]
    ...
    if ax_err is not None:
        y_scale_name = y_scale_name or _set_axis_scale_label_ticks(
            ...
            y_not_x=True,
            axis_label=(
                f"Logical Error Rate (per {failure_unit})" if yaxis is None else yaxis
            ),
            default_scale="log",
            forced_max_v=(
                max_y
                if max_y is not None
                else 1 if min_y is None or 1 > min_y else None
            ),
            default_min_v=1e-4,
            default_max_v=1,
            forced_min_v=min_y,
    ...
    if ax_dis is not None:
        ...
        ax_dis.set_yticks(
            [p / 10 for p in range(11)], labels=[f"{10*p}%" for p in range(11)]
        )
        ax_dis.set_ylim(0, 1)
    ...
    if subtitle is not None:
        if "{common}" in subtitle:
            auto_subtitle = ", ".join(
                f"{k}={v}"
                for k, v in sorted(_common_json_properties(plotted_stats).items())
            )
...
def main_plot(*, command_line_args: List[str]):
    ...
    if args.custom_error_count_keys:
        ...
        if missing:
            print(
                "Warning: the following custom error count keys didn't appear in any statistic:",
                file=sys.stderr,
            )
