    ...
    if dataclass_fields:
        for f in dataclasses.fields(cast(Any, obj)):
            if str(f.type).startswith("typing"):
                ...
                t = str(f.type)
            t = t.replace(
                "Union[Dict[str, ForwardRef('JSON_TYPE')], "
                "List[ForwardRef('JSON_TYPE')], str, int, float]",
                "Any",
            )
...
def generate_documentation(*, obj: object, level: int, full_name: str) -> Iterator[DescribedObject]:
    ...
    for sub_name in dir(obj):
        ...
        if full_name.endswith("str_svg"):
            pass
        if isinstance(obj, type) and sub_name not in obj.__dict__:
            continue
...
def main():
    objects = [
        obj
        for obj in generate_documentation(obj=stimflow, full_name="stimflow", level=0)
...
""".strip()
    ...
    for obj in objects:
        ...
        if len(obj.full_name.split(".")) > 2:
            ...
            print("# (at top-level in the stimflow module)")
        print("\n".join(obj.lines))
        print("```")
