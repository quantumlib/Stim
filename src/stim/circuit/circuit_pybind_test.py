...
def test_circuit_append_operation():
    ...
    c.append_operation(
        "OBSERVABLE_INCLUDE", [stim.target_rec(-1), stim.target_rec(-2)], 5
    )
    assert (
        str(c).strip()
        == """
...
'''))
    """.strip()
        == str(stim.CompiledMeasurementSampler(c))
    )
...
def test_circuit_generation():
    surface_code_circuit = stim.Circuit.generated(
        "surface_code:rotated_memory_z", distance=5, rounds=10
    )
...
def test_circuit_generation_errors():
    with pytest.raises(ValueError, match="Known repetition_code tasks"):
        stim.Circuit.generated("repetition_code:UNKNOWN", distance=3, rounds=1000)
    with pytest.raises(ValueError, match="Expected type to start with."):
        stim.Circuit.generated("UNKNOWN:memory", distance=0, rounds=1000)
    with pytest.raises(ValueError, match="distance >= 2"):
        stim.Circuit.generated("repetition_code:memory", distance=1, rounds=1000)
...
def test_search_for_undetectable_logical_errors_msgs():
    ...
    with pytest.raises(
        ValueError, match=r"NO OBSERVABLES(.|\n)*NO DETECTORS(.|\n)*NO ERRORS"
    ):
        c.search_for_undetectable_logical_errors(
            dont_explore_edges_increasing_symptom_degree=True,
            dont_explore_edges_with_degree_above=4,
...
q1: ---X-
    ...
    assert (
        str(c.diagram(type="detector-slice-text", tick=1)).strip()
        == """
q0: -Z:D0-
     |
q1: -Z:D0-
    ...
    assert (
        str(
            c.diagram(
                type="detector-slice-text",
                tick=1,
                filter_coords=[(5, 6, 7), (6,), (7, 8)],
            )
        ).strip()
        == """
q0: -Z:D1-
...
def test_detslice_ops_diagram_no_ticks_does_not_hang():
    assert (
        stim.Circuit.generated(
            "surface_code:rotated_memory_x", rounds=5, distance=5
        ).diagram("detslice-svg")
        is not None
    )
...
def test_reference_sample():
    ...
    np.testing.assert_array_equal(
        circuit.reference_sample(), circuit.reference_sample()
    )
...
def test_max_mix_depolarization_is_allowed_in_dem_conversion_without_args():
    assert (
        ...
        DETECTOR rec[-2]
    """
        ).detector_error_model(approximate_disjoint_errors=True)
        == stim.DetectorErrorModel(
            """
        error(0.5) D0
        error(0.5) D0 D1
...
def test_detslice_filter_coords_flexibility():
    ...
    d1 = c.diagram(
        ...
        filter_coords=[
            ...
            stim.DemTarget.relative_detector_id(5),
            "D7",
        ],
    )
...
def test_detecting_region_filters():
    ...
    assert (
        len(c.detecting_regions(targets=[stim.target_relative_detector_id(0), "D0"]))
        == 1
    )
...
def test_to_tableau():
    ...
    assert stim.Circuit("SPP_DAG Y0*Y1").to_tableau() == stim.Tableau.from_named_gate(
        "SQRT_YY_DAG"
    )
...
def test_circuit_eq_tags():
    assert stim.CircuitInstruction("TICK", tag="a") == stim.CircuitInstruction(
        "TICK", tag="a"
    )
    assert stim.CircuitInstruction("TICK", tag="a") != stim.CircuitInstruction(
        "TICK", tag="b"
    )
    assert stim.CircuitRepeatBlock(
        1, stim.Circuit(), tag="a"
    ) == stim.CircuitRepeatBlock(1, stim.Circuit(), tag="a")
    assert stim.CircuitRepeatBlock(
        1, stim.Circuit(), tag="a"
    ) != stim.CircuitRepeatBlock(1, stim.Circuit(), tag="b")
    assert (
        stim.Circuit(
            """
        H[test] 0
...
def test_circuit_get_item_tags():
    ...
    """
        ...
        == stim.CircuitInstruction("CX[test2] 1 2")
    )
    assert (
        stim.Circuit(
            """
    ...
    """
        ...
        == stim.CircuitRepeatBlock(3, stim.Circuit("M[test4](0.25) 4"), tag="test3")
    )
    assert (
        stim.Circuit(
            """
...
def test_tag_approx_equals():
    assert not stim.Circuit("H[test] 0").approx_equals(
        stim.Circuit("H[test2] 0"), atol=3
    )
...
def test_tag_detecting_regions():
    ...
    """
        ...
        == {stim.DemTarget("D0"): {0: stim.PauliString("Z")}}
    )
