...
def test_init_get():
    ...
    assert model[3] == stim.DemInstruction("shift_detectors", [1, 1.5, 2, 2.5], [1])
    assert model[4] == stim.DemInstruction("shift_detectors", [], [5])
...
def test_equality():
    ...
    assert stim.DetectorErrorModel("error(0.125) D0") == stim.DetectorErrorModel(
        "error(0.125) D0"
    )
    assert stim.DetectorErrorModel("error(0.125) D0") != stim.DetectorErrorModel(
        "error(0.126) D0"
...
def test_approx_equals():
    ...
    assert not base.approx_equals(
        stim.DetectorErrorModel("error(0.101) D0"), atol=0.00001
    )
    ...
    assert not base.approx_equals(
        stim.DetectorErrorModel("error(0.101) D0 D1"), atol=999
    )
...
def test_append_bad():
    ...
    with pytest.raises(
        ValueError,
        match=r"Bad target 'stim.DemTarget\('D0'\)' for instruction 'shift_detectors'",
    ):
        m.append(
            "shift_detectors", [0.125, 0.25], [stim.target_relative_detector_id(0)]
        )
...
def test_dem_from_file():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert stim.DetectorErrorModel.from_file(path) == stim.DetectorErrorModel(
            "error(0.125) D0 L5"
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(path, "w") as f:
            print("error(0.125) D0 L5", file=f)
        with open(path) as f:
            assert stim.DetectorErrorModel.from_file(f) == stim.DetectorErrorModel(
                "error(0.125) D0 L5"
            )
...
def test_init_parse():
    assert stim.DemInstruction("error(0.125) D0 D1") == stim.DemInstruction(
        "error", [0.125], [stim.DemTarget("D0"), stim.DemTarget("D1")]
    )
