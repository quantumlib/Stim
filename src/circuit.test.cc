...
TEST(circuit, operation_from_line) {
    ...
    ASSERT_EQ(f("DEPOLARIZE1(0.125) 4 5  # comment"), (Operation{"DEPOLARIZE1", OperationData({4, 5}, {false, false}, 0.125)}));
...
TEST(circuit, from_text) {
    ...
    ASSERT_EQ(Circuit::from_text("M 0 !0 1 !1"), Circuit(std::vector<Operation> {
        {"M", OperationData({0, 0, 1, 1}, {false, true, false, true}, 0)}}));
