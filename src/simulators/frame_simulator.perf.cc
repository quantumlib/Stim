...
BENCHMARK(FrameSimulator_depolarize1_100Kqubits_1Ksamples_per1000) {
    ...
    benchmark_go([&]() {
        sim.DEPOLARIZE1(targets, probability);
    }).goal_millis(5).show_rate("OpQubits", targets.size() * num_samples);
}
...
BENCHMARK(FrameSimulator_hadamard_100Kqubits_1Ksamples) {
    ...
    benchmark_go([&]() {
        sim.H_XZ(targets);
    }).goal_millis(2).show_rate("OpQubits", targets.size() * num_samples);
}
...
BENCHMARK(FrameSimulator_sample_unrotated_surface_code_d5_samples1024) {
    ...
    }).goal_micros(100).show_rate("Layers", distance * num_samples).show_rate("OutBits", circuit.num_measurements * num_samples);
}
...
BENCHMARK(FrameSimulator_sample_unrotated_surface_code_d41_samples1024) {
    ...
    benchmark_go([&]() {
        FrameSimulator::sample(circuit, ref, num_samples, rng);
    }).goal_millis(50).show_rate("Layers", distance * num_samples).show_rate("OutBits", circuit.num_measurements * num_samples);
}
