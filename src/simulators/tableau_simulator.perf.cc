...
BENCHMARK(TableauSimulator_sample_unrotated_surface_code_d5) {
    ...
    }).goal_micros(100).show_rate("Layers", distance).show_rate("OutBits", circuit.num_measurements);
}
...
BENCHMARK(TableauSimulator_sample_unrotated_surface_code_d41) {
    ...
    benchmark_go([&]() {
        TableauSimulator::sample_circuit(circuit, rng);
    }).goal_millis(420).show_rate("Layers", distance).show_rate("OutBits", circuit.num_measurements);
}
