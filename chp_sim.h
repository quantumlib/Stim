    ...
    void collapse_many(const std::vector<size_t> &targets, float bias) {
        ...
        for (auto target : collapse_targets) {
            for (size_t q = n - 1; q > 0; q--) {
                size_t victim = q >> 1;
                if (temp_transposed.z_obs_x_bit(target, q)) {
                    if (temp_transposed.z_obs_x_bit(target, victim)) {
                        temp_transposed.append_CX(victim, q);
                    } else {
                        temp_transposed.append_SWAP(victim, q);
                    }
                }
            }
            if (temp_transposed.z_obs_x_bit(target, 0)) {
                // Collapse the state.
                if (temp_transposed.z_obs_z_bit(target, 0)) {
                    temp_transposed.append_H_YZ(0);
                } else {
                    temp_transposed.append_H(0);
                }

                auto coin_flip = std::bernoulli_distribution(bias)(rng);
                if (temp_transposed.z_sign(target) != coin_flip) {
                    temp_transposed.append_X(0);
                }
            }
        }
...
//                    bool bx = cache_x[victim] & q1_mask;
//                    bool bz = cache_z[victim] & q1_mask;
//                    if (bx) {
//                        temp_transposed.append_CX(pivot, victim);
//                        cache_x[victim] ^= cache_x[pivot] & q1_mask;
//                        cache_z[pivot] ^= cache_z[victim] & q1_mask;
//                    }
////                    if (bx && bz) {
////                        temp_transposed.append_CY(pivot, victim);
////                        cache_z[pivot] ^= cache_x[victim] & q1_mask;
////                        cache_z[pivot] ^= cache_z[victim] & q1_mask;
////                        cache_x[victim] ^= cache_x[pivot] & q1_mask;
////                        cache_z[victim] ^= cache_x[pivot] & q1_mask;
////                    } else if (bx) {
////                        temp_transposed.append_CX(pivot, victim);
////                        cache_x[victim] ^= cache_x[pivot] & q1_mask;
////                        cache_z[pivot] ^= cache_z[victim] & q1_mask;
////                    } else if (bz) {
////                        temp_transposed.append_CZ(pivot, victim);
////                        cache_z[victim] ^= cache_x[pivot] & q1_mask;
////                        cache_z[pivot] ^= cache_x[victim] & q1_mask;
////                    }
//                }
...
//                    cache_z[pivot] ^= cache_x[pivot] & q1_mask;
//                } else {
//                    temp_transposed.append_H(pivot);
//                    cache_z[pivot] ^= cache_x[pivot] & q1_mask;
//                    cache_x[pivot] ^= cache_z[pivot] & q1_mask;
//                    cache_z[pivot] ^= cache_x[pivot] & q1_mask;
//                }
//
//                auto coin_flip = std::bernoulli_distribution(bias)(rng);
////                std::cerr << "FLIP " << q1 << ": ";
////                for (size_t k = 0; k < temp_transposed.tableau.num_qubits; k++) {
////                    if (temp_transposed.x_obs_z_bit(k, pivot)) {
////                        std::cerr << "X" << k << " ";
////                    }
////                    if (temp_transposed.z_obs_z_bit(k, pivot)) {
////                        std::cerr << "Z" << k << " ";
////                    }
////                }
////                std::cerr << "\n";
